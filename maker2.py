...
st.markdown("""
<style>
    /* [1] 기본 UI 초기화 및 전체 너비 축소 */
    header[data-testid="stHeader"] { display: none !important; }
    footer { display: none !important; }
    
    /* [수정] 전체 컨테이너 너비를 700px로 줄여서 입력창 너비를 축소시킴 */
    .block-container { 
        padding-top: 2rem !important; 
        padding-bottom: 5rem; 
        max-width: 700px !important; /* 1000px -> 700px 축소 */
    }
    ...
    /* 아이콘 위치 조정 (텍스트가 가운데라 아이콘이 겹치지 않게 우측 끝 유지 or 숨김 처리) */
    /* 여기서는 기본 유지하되 텍스트 중앙 정렬에 집중 */
    ...
    .q-label-container {
        ...
        font-weight: 700;
        color: #333;
        border-right: 2px solid #e0e6ed; /* 세로선 */
        padding-right: 15px;
        margin-right: 10px;
        padding-top: 10px;
    }
...
with c1: 
    input_val = st.text_input("custom_title_input", placeholder="문제집 이름 입력", label_visibility="collapsed")
...
if available_exams:
    ...
    for i in range(1, st.session_state.target_q_count + 1):
        ...
        # [구조] 좌측 라벨(1.5) | 우측 입력창(8.5) - 비율 살짝 조정하여 좁아진 폭에 대응
        col_label, col_input = st.columns([1.5, 8.5], gap="small")
    ...
    foot_label, foot_btns = st.columns([1.5, 8.5], gap="small")
